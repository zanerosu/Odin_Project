...
btn_create.addEventListener("click", () => {
  if (!input_title.validity.valid) {
    console.log("Please enter valid title!");
  } else if (!input_author.validity.valid) {
    console.log("Please enter valid author!");
  } else if (!input_pages.validity.valid) {
    console.log("Please enter valid pages!");
  } else {
    event.preventDefault();
    new_book_dialog.close("New book created");
    page_content.style.filter = "brightness(100%)";
    addBookToLibrary();
    displayBooks();
...
function validateForm() {
  ...
  title.addEventListener("input", (event) => {
    if (title.validity.typeMismatch) {
      title.setCustomValidity("I am expecting a text type!");
    } else {
  ...
  pages.addEventListener("input", (event) => {
    if (pages.validity.rangeUnderflow) {
      pages.setCustomValidity("There must be at least 1 or more pages!");
    } else {
